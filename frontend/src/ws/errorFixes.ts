...
export interface ErrorFix {
  /** 一句话说明怎么修。 */
  action: string;
  /** 可选：点击后跳转的设置分区（Dashboard 三层导航 id）。 */
  section?: 'llm' | 'voice' | 'memory' | 'screen' | 'proactive' | 'agent' | 'general';
}
...
const FIX_MAP: Partial<Record<ErrorCode, ErrorFix>> = {
  ...
  LLM_UNREACHABLE: {
    action: '无法连接对话模型：检查网络，或在「高级 → 对话大脑」里重新测试配置。',
    section: 'llm',
  },
  LLM_INVALID_KEY: {
    action: 'API Key 无效或未配置：到「高级 → 对话大脑」检查你的 Key。',
    section: 'llm',
  },
  LLM_TIMEOUT: { action: '对话模型响应超时：试试换小一点的模型，或稍后再聊。', section: 'llm' },
  TTS_FAILED: { action: '语音合成失败：到「用户 → 声音 TTS / ASR」检查音色与引擎配置。', section: 'voice' },
  ...
  CONFIG_INVALID: {
    action: '配置文件校验失败：可在「高级」里检查最近改动的设置，必要时恢复默认配置。',
    section: 'general',
  },
  MCP_SERVER_FAILED: { action: '工具 / MCP 服务器异常：在「开发者 → 工具 MCP」里检查。', section: 'agent' },
  MEMORY_FAILED: { action: '记忆系统异常：到「高级 → 记忆」查看或重置记忆。', section: 'memory' },
