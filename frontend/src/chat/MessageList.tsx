...
export function MessageList({
  ...
  const thinkingName = state.confName || 'Moonlight';
  ...
  const orphanRuns = (taskRuns ?? []).filter(
    (r) => r.anchorId == null || !messages.some((m) => m.id === r.anchorId),
  );
  ...
  return (
    <div className="message-list" ref={scrollRef}>
      ...
      {messages.map((msg) => (
        <Fragment key={msg.id}>
          <ChatBubble message={msg} onReplayAudio={onReplayAudio} />
          {/* 2026-08-10 v5：按 run 分散内联到各自锚点消息之后（与请求保持上下文） */}
          {(taskRuns ?? [])
            .filter((r) => r.anchorId === msg.id)
            .map(renderCard)}
        </Fragment>
      ))}
