...
class ServiceContext:
    ...
    async def construct_system_prompt(self, persona_prompt: str) -> str:
        ...
        # 注入好感度/關係狀態（見 affection.py）：依目前關係親疏引導語氣。
        # 完全 fail-soft：任何錯誤都不會擋住對話。
        try:
            from .affection import affection_context_block

            _rel = affection_context_block(self.character_config.conf_uid)
            if _rel:
                persona_prompt += _rel
        except Exception as _rel_e:
            logger.warning(f"[affection] prompt injection failed: {_rel_e}")
