...
class LLMTranslateConfig(I18nMixin):
    ...
    # 可选鉴权 key（如 DeepSeek 等云端 API 需要 Bearer；本地 Ollama 留空）。
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "llm_api_key": Description(
            en="Optional API key for cloud LLM endpoints (empty for local Ollama)",
            zh="雲端 LLM 端點的 API key（可選；本地 Ollama 留空）",
        ),
    }
