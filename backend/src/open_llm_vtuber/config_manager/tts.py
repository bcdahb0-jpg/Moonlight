...
class VoiceVoxTTSConfig(I18nMixin):
    """Configuration for local VOICEVOX TTS (Japanese, incl. ずんだもん loli voice)."""

    base_url: str = Field("http://127.0.0.1:50021", alias="base_url")
    speaker: int = Field(3, alias="speaker")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "base_url": Description(
            en="URL of the local VOICEVOX engine (default http://127.0.0.1:50021)",
            zh="本地 VOICEVOX 引擎地址（默认 http://127.0.0.1:50021）",
        ),
        "speaker": Description(
            en="VOICEVOX speaker ID (3 = ずんだもん normal, 4 = ずんだもん sweet, 8 = 春日部つむぎ)",
            zh="VOICEVOX 说话人 ID（3=ずんだもん 普通，4=ずんだもん 甜甜，8=春日部つむぎ）",
        ),
    }
