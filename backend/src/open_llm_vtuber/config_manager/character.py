...
class CharacterConfig(I18nMixin):
    ...
    # Memory v2 (memory_v2.py): typed facts/reflections + dreaming merge (default ON).
    # When on, each turn's facts are extracted post-turn (fire-and-forget LLM call),
    # reflections synthesize from K facts, and high-confidence reflections are fused
    # into core_memory.md by the dreaming pass. Turning it off skips extraction AND
    # the facts/reflections retrieval injection (core memory still works as before).
    memory_v2_enabled: bool = Field(default=True, alias="memory_v2_enabled")
    # Per-character fact cap for the memory_v2 facts table. Default 500; bounded to
    # [50, 5000] by the validator below.
    memory_v2_max_facts: int = Field(
        default=500, alias="memory_v2_max_facts"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "memory_v2_enabled": Description(
            en="Enable memory v2 (typed facts + reflections + dreaming merge). "
            "Extracts facts post-turn, synthesizes reflections, and fuses "
            "high-confidence reflections into core memory on the dreaming pass.",
            zh="啟用記憶 v2（類型化事實 + 反思 + 睡眠合併）。"
            "每輪背景抽取事實、合成反思、dreaming 把高置信反思精選寫入核心記憶。",
        ),
        "memory_v2_max_facts": Description(
            en="Max facts stored per character (50-5000, default 500). "
            "Over the cap, least important/oldest facts are archived first.",
            zh="每角色 facts 上限（50–5000，預設 500）。"
            "超過上限時先歸檔最不重要/最舊的事實。",
        ),
    }
    ...
    @field_validator("memory_v2_max_facts")
    def clamp_memory_v2_max_facts(cls, v):
        # Mirror memory_v2.FACT_MAX_DEFAULT; bounded [50, 5000]. Fail-soft to 500.
        try:
            n = int(v)
        except (TypeError, ValueError):
            return 500
