...
# 模块级句柄：指向当前活跃的 WebSocketHandler，供其他组件（如 MCP 服务端）
# 访问已连接前端（client_connections），避免强耦合。
_ws_handler_instance: Optional["WebSocketHandler"] = None


def set_ws_handler(handler: "WebSocketHandler") -> None:
    """注册当前活跃的 WebSocketHandler 实例。"""
    global _ws_handler_instance
    _ws_handler_instance = handler


def get_ws_handler() -> Optional["WebSocketHandler"]:
    """返回已注册的 WebSocketHandler 实例，未注册则返回 None。"""
...
class WebSocketHandler:
    ...
    async def _send_initial_messages(
        ...
        # Start microphone
        await send_message(websocket.send_text, {"type": "control", "text": "start-mic"})
        ...
        try:
            while True:
                try:
                    ...
                    if invalid_reason:
                        logger.warning(
                            f"Rejected inbound message from {client_uid}: {invalid_reason}"
                        )
                        await send_error(
                            websocket.send_text,
                            ErrorCode.INVALID_MESSAGE,
                            f"消息被拒绝：{invalid_reason}",
                        )
                        continue
                    ...
                    # 连接已不可用（客户端断开 / 未 accept / 竞态）时，receive_json
                    # 会反复抛 "WebSocket is not connected" 类错误。若在此 continue，
                    # 该任务会在 while True 里无限空转刷错误日志，占满 event loop，
                    # 导致 HTTP API 全部饿死（后端「假死」，角色卡/模型库加载不出来）。
                    # 规则：连接层错误直接终止本连接；业务错误才走 send_error + continue。
    ...
    async def _handle_interrupt(
        ...
        # 关键：无论是否命中运行中的任务，都必须回发 interrupt 控制信号，
        # 否则前端 thinking 状态无法复位（一直转圈、无法停止）。
        await send_message(
            websocket.send_text,
            {"type": "control", "text": "interrupt"},
        )
    ...
    async def _handle_history_list_request(
        ...
        histories = get_history_list(
            ...
            keep_uid=context.history_uid,
        )
        await send_message(
            websocket.send_text, {"type": "history-list", "histories": histories}
        )
    ...
    async def _handle_set_history_title(
        self, websocket: WebSocket, client_uid: str, data: dict
    ):
        """Handle renaming a chat history (custom session title)"""
        history_uid = data.get("history_uid")
        ...
        context = self.client_contexts[client_uid]
        # 自定义标题也做个长度保护（避免异常超长标题撑爆侧边栏）
        title = title[:30]
        success = bool(title) and update_metadate(
            context.character_config.conf_uid,
            history_uid,
            {"title": title},
        )
        ...
        if chunk:
            for audio_bytes in context.vad_engine.detect_speech(chunk):
                if audio_bytes == b"<|PAUSE|>":
                    await send_message(
                        websocket.send_text, {"type": "control", "text": "interrupt"}
                    )
    ...
    async def _handle_interact(
        ...
        """养成交互：前端点击/摸头等互动事件 -> 随机点击台词 + 少量好感度。

        完全 fail-soft：任何错误都不影响连接。zone（head/body/click）目前仅日志区分，
        统一走「点击台词」场景。
        """
        try:
            context = self.client_contexts[client_uid]
            ...
            from .affection import add_affection, affection_summary

            _zone = (data.get("zone") or "click").strip()
            logger.info(f"[interact] {_zone} on {conf_uid}")
            _q = random_quote(conf_uid, "click")
            if _q:
                await send_message(
                    websocket.send_text,
                    {"type": "full-text", "text": _q, "quote": True},
                )
            # 互动小加分（套用每日上限）
            add_affection(conf_uid, 1)
            await send_message(
                websocket.send_text,
                {
                    "type": "affection-update",
                    "affection": affection_summary(conf_uid),
                },
            )
        except Exception as e:
            logger.warning(f"[interact] failed: {e}")
    ...
    async def _handle_fetch_configs(
        ...
        await send_message(
            websocket.send_text, {"type": "config-files", "configs": config_files}
        )
    ...
    async def _handle_fetch_backgrounds(
        ...
        await send_message(
            websocket.send_text, {"type": "background-files", "files": bg_files}
        )
