...
# TTS voice catalogs: per-engine voice picker for the character card.
from .tts.tts_voice_catalog import (
    CURATED_VOICES,
    ENGINE_INT_VOICE,
    ENGINE_VOICE_FIELD,
    get_voice_catalog,
...
# --------------------------------------------------------------------------- #
# CURATED_VOICES is imported from tts_voice_catalog (single source of truth).
...
VOICE_SAMPLE_TEXTS = {
    "zh-TW": "月光落在窗臺上，晚風輕拂過耳畔，想與你說聲晚安。",
    "zh-CN": "月光落在窗台上，晚风轻拂过耳畔，想与你说声晚安。",
    "ja": "月が綺麗ですね。風が優しく吹いています。",
    "en": "The moon is beautiful tonight. May your dreams be gentle.",
}
VOICE_SAMPLE_DEFAULT_TEXT = "The moon is beautiful tonight. May your dreams be gentle."

# /api/tts-voice-sample 的默认试听文本（文艺向）：VOICEVOX 是日语引擎，用日语；
# 其余引擎（含 edge/siliconflow 等中文引擎）用中文。
VOICE_SAMPLE_DEFAULT_ZH = "月光落在窗台上，晚风轻拂过耳畔，想与你说声晚安。"
VOICE_SAMPLE_DEFAULT_JA = "月が綺麗ですね。風が優しく吹いています。"
...
def _read_character_fields(path: str, *, is_base: bool) -> Optional[dict]:
    ...
    tts = cc.get("tts_config") or {}
    if not isinstance(tts, dict):
        tts = {}
    # 角色覆盖的 TTS 引擎；无覆盖时为 None（继承基础配置）
    ...
    voice = None
    if tts_model:
        field = ENGINE_VOICE_FIELD.get(tts_model)
        if field:
            try:
                raw = tts.get(tts_model, {}).get(field)
            except AttributeError:
                raw = None
            if raw is not None:
                voice = str(raw)
    # 兼容旧数据：无 tts_model 但有旧 edge_tts.voice
    if voice is None:
        try:
            legacy = tts.get("edge_tts", {}).get("voice")
        except AttributeError:
            legacy = None
...
def _build_tts_config(tts_model: str, voice: Optional[str]) -> dict:
    """Build a tts_config block for a character: {tts_model, <engine>: {<field>: voice}}.

    voice 字段名由 ENGINE_VOICE_FIELD 决定；int 引擎（voicevox/sherpa/piper）做 int 转换。
    voice 为空时只写引擎名，引擎子块通过 deep_merge 继承基础配置。
    """
    block: dict = {"tts_model": tts_model}
    field = ENGINE_VOICE_FIELD.get(tts_model)
    if field and voice:
        value: object = voice
        if tts_model in ENGINE_INT_VOICE:
            try:
                value = int(str(voice))
            except (TypeError, ValueError):
                value = voice
        block[tts_model] = {field: value}
    return block


def _write_engine_voice(tts: dict, engine: str, voice: Optional[str]) -> None:
    """In-place: write voice into tts[engine][field] (creating the sub-block)."""
    field = ENGINE_VOICE_FIELD.get(engine)
    if not field or not voice:
        return
    value: object = voice
...
def _build_character_config(
    *,
    ...
    base via deep_merge on switch. tts_config is OMITTED when neither engine nor
    voice is set so the character re-inherits the base voice.
    """
    cc: dict = {
        "conf_name": conf_name,
        "conf_uid": conf_uid,
    ...
    if avatar:
        cc["avatar"] = avatar
    if tts_model:
        cc["tts_config"] = _build_tts_config(tts_model, voice)
    elif voice:
        # 兼容旧数据：没有引擎时默认 edge_tts
        cc["tts_config"] = _build_tts_config("edge_tts", voice)
    return cc
...
def _update_base_character_config(
    *,
    ...
    if avatar is not None:
        cc["avatar"] = avatar
    # voice / engine: when a tts_model is provided, write it + the engine's voice
    # field; otherwise keep the legacy edge_tts behavior. Never blow away the
    # existing tts_config block.
    if tts_model:
        ...
        if tts is None:
            ...
            _write_engine_voice(tts, tts_model, voice)
    elif voice:
        tts = cc.get("tts_config")
        if tts is None:
...
def _extract_body_fields(body: dict) -> dict:
    ...
    tts_model = body.get("tts_model") or body.get("tts_engine")
    slug = body.get("slug")
    ...
    return {
        ...
        "tts_model": (tts_model.strip() if isinstance(tts_model, str) else tts_model),
        "slug": slug,
...
def init_character_route() -> APIRouter:
    ...
    # ------------------------------------------------------------------ #
    @router.post("/api/live2d-skins/thumbnail")
    async def save_skin_thumbnail(request: Request):
        """Save a rendered PNG preview for a Live2D model as live2d-models/<name>.png.

        The frontend renders the model (pixi-live2d-display) to a canvas and POSTs a
        PNG data URL here; we persist it next to the model. Both /live2d-models/info
        (routes.py) and _find_skin_thumbnail pick up ``<name>.png`` as the model's
        card/avatar image, so character cards show a real full-character render
        instead of the raw texture atlas.
        """
        if not _is_local_request(request):
            return _forbidden()
        try:
        ...
        if not name or not isinstance(data, str) or not data.startswith("data:"):
            return _bad_request("Missing model name or image data.")

        # The folder must exist under live2d-models/ and name must be a bare dir name.
        folder = os.path.join(LIVE2D_DIR, name)
        base = os.path.normpath(os.path.abspath(LIVE2D_DIR))
        folder_norm = os.path.normpath(os.path.abspath(folder))
        if not (folder_norm == base or folder_norm.startswith(base + os.sep)):
            return _bad_request("Invalid model name.")
        if not os.path.isdir(folder):
            return _bad_request("Model folder not found.")
        ...
        try:
            ...
            if "image/png" not in header:
                return _bad_request("Expected a PNG data URL.")
            raw = _b64.b64decode(payload, validate=True)
            if not raw:
                return _bad_request("Empty image data.")
        except Exception:
            return _bad_request("Invalid PNG data.")

        dest = os.path.join(folder, f"{name}.png")
        tmp = os.path.join(folder, f".{name}.png.tmp")
        try:
            with open(tmp, "wb") as f:
                f.write(raw)
            os.replace(tmp, dest)
        except Exception as e:
            logger.error(f"skin thumbnail write failed ({name}): {type(e).__name__}")
            return JSONResponse(status_code=500, content={"error": "write failed"})

        logger.info(f"skin thumbnail saved: {name}.png")
    ...
    async def list_tts_voices(request: Request):
        """Per-engine TTS voice catalog for the character-card voice picker.

        GET /api/tts-voices?engine=minimax_tts ->
            {engine, mode: "list"|"input", voices: [{value,label}], input: {...}|null}
        """
        if not _is_local_request(request):
            return _forbidden()
        engine = (request.query_params.get("engine") or "").strip()
        if not engine:
            return _bad_request("Missing engine parameter.")
        catalog = await get_voice_catalog(engine)
        return JSONResponse(catalog)

    # ------------------------------------------------------------------ #
    @router.get("/api/tts-voice-sample")
    async def tts_voice_sample(request: Request):
        """用真实 TTS 引擎 + 指定音色合成一句试听音频（試聽）。

        GET /api/tts-voice-sample?engine=edge_tts&voice=zh-TW-HsiaoYuNeural
        返回音频字节（media type 由引擎输出格式决定）。失败返回 502。
        """
        if not _is_local_request(request):
            return _forbidden()
        engine = (request.query_params.get("engine") or "").strip()
        ...
        if not engine or not voice:
            return _bad_request("Missing engine or voice.")
        if not text:
            text = (
                VOICE_SAMPLE_DEFAULT_JA
                if engine == "voicevox_tts"
                else VOICE_SAMPLE_DEFAULT_ZH
            )
        else:
            text = text[:VOICE_SAMPLE_MAX_TEXT]
        audio = await synthesize_voice_sample(engine, voice, text)
        if not audio:
            return JSONResponse(
                status_code=502,
                content={
                    ...
                    "(engine/service may be unavailable, or credentials missing).",
                },
            )
        data, ext = audio
        return Response(
...
# --------------------------------------------------------------------------- #
# _live_edge_voices() is imported from tts_voice_catalog (single source of truth).
