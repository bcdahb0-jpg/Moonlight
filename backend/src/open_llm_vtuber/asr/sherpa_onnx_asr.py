...
_t2s_converter = None
_t2s_converter_failed = False


def _to_simplified(text: str) -> str:
    """把 ASR 輸出的繁體/混合中文轉成簡體；英文／其他語言原樣通過。失敗時回傳原文。"""
    global _t2s_converter, _t2s_converter_failed
    if not text or _t2s_converter_failed:
        return text
    try:
        ...
        _t2s_converter_failed = True
        logger.warning(f"opencc t2s 繁轉簡失敗，改用原始辨識文字：{e}")
        return text
...
class VoiceRecognition(ASRInterface):
    def __init__(
        ...
        simplified_output: bool = True,  # 识别结果统一转为简体中文（false = 保持模型原始输出）
    ) -> None:
