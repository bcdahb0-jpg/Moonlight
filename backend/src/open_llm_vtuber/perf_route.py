...
# TTS engines the UI offers. init_tts fails soft to edge_tts when an engine can't
# load (missing dep / unreachable server), so offering the full set is safe.
TTS_MODELS = {
    "edge_tts", "gpt_sovits_tts", "azure_tts", "bark_tts", "cosyvoice_tts",
    "cosyvoice2_tts", "melo_tts", "coqui_tts", "x_tts", "fish_api_tts",
...
def _tts_from_conf() -> dict:
    ...
    ``tts_voice`` is the voice the ACTIVE engine will actually use (keyed by
    tts_model), so the UI can show "which voice is in effect" and de-confuse the
    per-character edge-tts picker vs the engine-level voice.
    """
    out = {
        "tts_model": "edge_tts",
        "gpt_sovits_api_url": "",
    ...
    try:
        ...
        # 当前引擎的实际音色
        eng = str(out["tts_model"])
        blk = tts.get(eng, {}) or {}
        if eng == "edge_tts":
            out["tts_voice"] = str(blk.get("voice") or "")
        elif eng == "siliconflow_tts":
            out["tts_voice"] = str(blk.get("default_voice") or "")
        elif eng == "voicevox_tts":
            out["tts_voice"] = "speaker=" + str(blk.get("speaker") or "")
        elif eng == "gpt_sovits_tts":
            out["tts_voice"] = str(blk.get("ref_audio_path") or "") or str(
                blk.get("api_url") or ""
            )
        else:
            out["tts_voice"] = str(blk.get("voice") or "")
