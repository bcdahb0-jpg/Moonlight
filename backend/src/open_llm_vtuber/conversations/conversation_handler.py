...
async def handle_conversation_trigger(
    ...
    if msg_type == "ai-speak-signal":
        ...
        # 关键词触发台词：命中关键词立即弹一句预设台词（秒级响应），AI 再照常回复。
        # 完全 fail-soft：任何错误都不影响对话。
        if isinstance(user_input, str) and user_input.strip():
            try:
                from ..quotes import keyword_trigger

                _quote = keyword_trigger(
                    context.character_config.conf_uid, user_input
                )
                if _quote:
                    await send_message(
                        websocket.send_text,
                        {"type": "full-text", "text": _quote, "quote": True},
                    )
